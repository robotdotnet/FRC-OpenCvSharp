...
namespace OpenCvSharp.FRC
{
    ...
    internal class NativeList<T> : IList<T>, System.Collections.IList, IReadOnlyList<T>
    {
        ...
        {
            if (capacity < 0) 
                throw new ArgumentOutOfRangeException(nameof(capacity), "Capacity needs to be positive");
        ...
        public int Capacity
        {
            ...
            set
            {
                if (value < _size)
                {
                    throw new ArgumentOutOfRangeException(nameof(value), "Capacity is smaller then size");
                }
                //Contract.End//ContractBlock();
                
                // Only resize if array is smaller
        ...
        {
            if (item == null)
                    throw new ArgumentOutOfRangeException(nameof(item), "Value cannot be null");
        ...
        {
            if (index < 0)
                throw new ArgumentOutOfRangeException(nameof(index), "Need non negative index");
            if (count < 0)
                throw new ArgumentOutOfRangeException(nameof(count), "Need non negative count");
        ...
        /*
        public NativeList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter) where TOutput : struct 
        {
            if (converter == null)
            {
        ...
        */
        ...
        void System.Collections.ICollection.CopyTo(Array array, int arrayIndex)
        {
            ...
            {
                throw new ArgumentException("Multi dimmentional rank not supported");
            }
            //Contract.End//ContractBlock();
        ...
        public void ForEach(Action<T> action)
        {
            ...
            if (version != _version)
                throw new InvalidOperationException("Enumerator has failed");
        }
        ...
        public int IndexOf(T item, int index, int count)
        {
            ...
            if (count < 0 || index > _size - count) throw new ArgumentOutOfRangeException(nameof(count), "Count out of range");
            //Contract.Ensures(//Contract.Result<int>() >= -1);
            //Contract.Ensures(//Contract.Result<int>() < Count);
            //Contract.End//ContractBlock();
        ...
        public void Insert(int index, T item)
        {
            ...
            if ((uint)index > (uint)_size)
            {
                throw new ArgumentOutOfRangeException(nameof(index), "List insert argument out of range");
            }
            //Contract.End//ContractBlock();
            if (_size == _items.Length) EnsureCapacity(_size + 1);
            if (index < _size)
        ...
        {
            if (item == null)
                throw new ArgumentNullException(nameof(item), "Item cannot be null to inset");
        ...
        public int LastIndexOf(T item, int index, int count)
        {
            ...
            if (index >= _size)
            {
                throw new ArgumentOutOfRangeException(nameof(index), "Index is bigger then list size");
            }

            if (count > index + 1)
            {
                throw new ArgumentOutOfRangeException(nameof(count), "Count is bigger then list size");
            }
        ...
        public struct Enumerator : IEnumerator<T>, System.Collections.IEnumerator
        {
            ...
            Object System.Collections.IEnumerator.Current
            {
                get
                {
                    ...
                    {
                        throw new InvalidOperationException("Enumerator operation cannot happen");
                    }
                    return Current;
                }
            }
